...
/*
    ...
    - thisRef, 需要属性拥有者相同的类型或是其父类型（对于扩展属性来说，这个类型指的是被扩展的那个类型）
    ...
    getValue与setValue方法既可以作为委托类的成员方法实现，也可以作为扩展方法实现

    这两个方法都必须要标记为operator关键字，对于委托来说，他可以实现readOnProperty或是ReadWriteProperty接口，
    这些接口包含了相应的getValue与setValue方法：同时，对于委托类来说，也可以不去实现这两个接口，而是自己单独实现相应的getValue与setValue方法

    委托转换规则：
    对于每个委托属性来说，Kotlin编译器在底层会生成一个辅助的属性，然后将对原有属性的访问委托给这个辅助属性。
    比如说： 对于属性prop来说，kotlin编译器所生成的隐含的属性名为prop$delegate，然后对原有的prop属性的访问器的
    访问都只是委托给这个额外的，kotlin编译器所生成的辅助属性。
 */
